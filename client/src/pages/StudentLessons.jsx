...
function StudentLessons() {
  ...
  const handleEnroll = async () => {
    try {
      ...
      if (
        ...
        error.response.data.message.includes("já está matriculado")
      ) {
  ...
  const handleUnenroll = async () => {
    if (
      window.confirm(
        "Tem certeza que deseja cancelar sua matrícula neste curso? Todo o progresso será perdido."
      )
    ) {
      try {
        await axios.post(
  ...
  const fetchCompletedLessons = async (teacherId) => {
    try {
      const res = await axios.get(
        `${API_URL}/api/lessons/completed/${teacherId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      <main className="flex-grow p-4 md:p-8 flex flex-col lg:flex-row gap-8 overflow-hidden h-screen">
        <aside className="w-full lg:w-1/4 bg-gray-800 rounded-lg p-4 flex flex-col gap-4 overflow-y-auto max-h-full shadow-lg">
          <div className="flex gap-2 mb-2 p-1 bg-gray-900 rounded-lg shrink-0">
            <button
              ...
              className={`flex-1 py-2 rounded-md font-bold text-sm transition-all duration-300 ${
                activeTab === "enrolled"
                  ? "bg-sidebar-bg text-white shadow-md"
          ...
          <div
            ...
            data-testid="course-list-container"
          >
            {isLoading ? (
          ...
          {selectedCourse ? (
            <>
                  ...
                  <div className="flex justify-between items-center gap-4">
                    ...
                    {selectedCourse.is_enrolled ? (
                      <div className="flex gap-2">
                        <button
                          onClick={openChat}
                          className="relative bg-blue-600 hover:bg-blue-700 py-2 px-4 rounded-lg font-bold transition-colors shadow-md text-white flex items-center gap-2 text-sm"
                        >
                          💬 Falar com Prof.
                        ...
                        <button
                          ...
                          className="bg-red-600 hover:bg-red-700 py-2 px-4 rounded-lg font-bold transition-colors shadow-md text-white text-sm"
                        >
                          Desmatricular
                        </button>
                      </div>
                      ...
                      <button
                        ...
                        className="bg-green-600 hover:bg-green-700 py-2 px-6 rounded-lg font-bold transition-colors shadow-md text-white text-sm shadow-[0_0_10px_rgba(22,163,74,0.4)] animate-pulse"
                      >
                        Inscrever-se Grátis
                      </button>
                    )}
                ...
                {selectedCourse.is_enrolled ? (
                  activeClass ? (
                    <div className="flex flex-col gap-4">
                      <div className="w-full aspect-video bg-black rounded-xl overflow-hidden border border-gray-700 shadow-2xl">
                        <iframe
                          ...
                          src={activeClass.video_url?.replace(
                            "watch?v=",
                            "embed/"
                          )}
                          allowFullScreen
                      ...
                      <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <button
                            onClick={() =>
                              openReviewModal(activeClass, "course_class")
                            }
                            className="bg-yellow-500 hover:bg-yellow-600 text-black font-bold px-4 py-2 rounded-lg text-sm shadow-md transition-colors flex items-center gap-2"
                          >
                            ⭐ Avaliar Aula
                          </button>
                        </div>
                        ...
                        <p className="text-gray-300 text-sm">
                          {activeClass.description || "Nenhuma descrição."}
                        </p>

                        {activeClass.documents &&
                          activeClass.documents.length > 0 && (
                            <div className="mt-6 pt-4 border-t border-gray-700">
                              <h3 className="font-bold text-purple-300 mb-3 text-sm">
                              ...
                              <div className="flex flex-wrap gap-3">
                                {activeClass.documents.map((doc) => (
                                  <a
                                    ...
                                    className="bg-gray-700 hover:bg-gray-600 px-3 py-1.5 rounded border border-gray-600 text-xs font-semibold flex items-center gap-2 transition text-white"
                                  >
                                    📄 {doc.name}
                                  </a>
                                ))}
                    ...
                    <div className="flex-grow flex items-center justify-center bg-gray-800 rounded-lg border border-gray-700 text-gray-500 p-6 text-center">
                      <p>
                        Nenhuma aula selecionada ou o curso ainda não possui
                        conteúdo.
                      </p>
                    </div>
                  )
                  ...
                  <div className="flex-grow bg-gray-800 rounded-lg p-8 shadow-lg border border-gray-700 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-sm text-yellow-400 font-semibold mb-4">
                      Veja o conteúdo do curso na aba ao lado 👉
                    </p>
                  </div>
                )}
                ...
                {selectedCourse.is_enrolled && completedLessons.length > 0 && (
                  <div className="bg-gray-800 rounded-lg p-6 border border-gray-700 mt-4 shrink-0">
                    <p className="text-sm text-gray-400 mb-3 font-semibold">
                      Pendentes de Avaliação (Aulas ao Vivo):
                    </p>
                    ...
                    <div className="flex flex-col gap-2">
                      {completedLessons.map((lesson) => (
                        <div
                          key={lesson.id}
                          className="flex items-center justify-between bg-gray-900 px-4 py-3 rounded-lg border border-gray-700"
                        >
                          ...
                          <button
                            ...
                            className="text-xs bg-yellow-500 hover:bg-yellow-600 text-black font-bold px-3 py-1.5 rounded-lg transition-colors"
                          >
                            ⭐ Avaliar
                          </button>
                        </div>
              ...
              <aside className="w-full lg:w-72 bg-[#1a1a1a] rounded-xl border border-gray-700 flex flex-col shadow-xl flex-shrink-0">
                <div className="p-4 border-b border-gray-700 bg-gray-900 rounded-t-xl">
                  <h2 className="font-bold text-base text-purple-300">
                    Conteúdo do Curso
                  </h2>
                </div>
                            ...
                            <button
                              onClick={() =>
                                selectedCourse.is_enrolled
                                  ? setActiveClass(cls)
                                  : alert(
                                      "Matricule-se no curso para assistir as aulas!"
                                    )
                              }
                              className={`w-full text-left p-3 pr-10 rounded-lg text-sm transition-colors border ${
            ...
            <div className="flex-grow flex items-center justify-center bg-gray-800 rounded-lg">
              <p className="text-gray-400">
                Selecione um curso na lista para visualizar o conteúdo.
              </p>
            </div>
          )}
        </section>
