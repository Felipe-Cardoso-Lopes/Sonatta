...
// 2. INICIALIZA O SOCKET APENAS UMA VEZ AO MONTAR A TELA
  useEffect(() => {
    ...
    // SOLUÇÃO: Comparações forçadas com String()
  ...
  // 3. ATUALIZA O CHAT AUTOMATICAMENTE AO MUDAR DE CURSO OU ABRIR O MODAL
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-dark-bg text-white-text font-poppins flex flex-col md:flex-row relative">
      <div className="shrink-0 z-20"><StudentSidebar /></div>
          ...
          <div className="flex flex-col gap-4">
            {isLoading ? (
              ...
              courses.filter(c => c.is_enrolled).length === 0 ? (
                ...
                courses.filter(c => c.is_enrolled).map(course => <CourseCard key={course.id} {...course} isSelected={selectedCourse?.id === course.id} onClick={() => setSelectedCourse(course)} />)
              )
            ) : (
              courses.filter(c => !c.is_enrolled).length === 0 ? (
              ...
              <div className="flex-grow bg-gray-800 rounded-lg p-6 shadow-lg border border-gray-700">
                ...
                {selectedCourse.is_enrolled ? (
                  <div className="flex gap-4 mb-6 mt-6">
                    ...
                    <button onClick={openChat} className="relative bg-blue-600 hover:bg-blue-700 py-3 px-6 rounded-lg font-bold transition-colors shadow-md text-white flex items-center gap-2">
                      💬 Falar com Prof.
                ...
                <div><p className="text-sm text-gray-300 mt-3">{selectedCourse.description}</p></div>
              </div>
            </>
          ) : (
            <div className="flex-grow flex items-center justify-center bg-gray-800 rounded-lg"><p className="text-gray-400">Selecione um curso.</p></div>
          )}
        </section>
      </main>
          ...
          <div className="bg-blue-600 p-3 flex justify-between items-center text-white shadow-md z-10">
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                {isTeacherOnline && <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-400 border border-blue-600 rounded-full"></span>}
              </div>
              <div>
                <h3 className="font-bold text-sm leading-tight">Prof. {nomeProfSelected}</h3>
                <p className="text-[10px] text-blue-200">{isTeacherOnline ? 'Online' : 'Offline'}</p>
              </div>
            </div>
            ...
            {isTeacherTyping && (
              <div className="flex justify-start">
                <div className="p-2 rounded-xl bg-gray-700 text-gray-400 rounded-bl-none text-xs italic flex gap-1">
                  Digitando<span className="animate-bounce">.</span><span className="animate-bounce delay-100">.</span>
                </div>
              </div>
            )}
          </div>
